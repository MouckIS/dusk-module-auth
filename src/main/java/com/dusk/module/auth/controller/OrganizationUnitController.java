...
public class OrganizationUnitController extends CruxBaseController {
    ...
    @Operation(summary = "获取组织机构下的用户(部分信息)")
    ...
    @Operation(summary = "组织机构的用户选择下拉列表")
    ...
    @Operation(summary = "获取组织机构下的用户, 返回键值对数据")
    ...
    @Operation(summary = "根据用户ID获取该用户所属的所有组织机构")
    @Parameters({
    ...
    @Operation(summary = "获取组织机构节点的父级[包括自身]")
    ...
    @Operation(summary = "获取当前用户下的所有厂站结构树-（包含普通节点的最小树）")
